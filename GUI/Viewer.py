...
class Viewer(QMainWindow):
    ...
    def addImage(self, name, fileName, display=True, addMenu=False):
        ...
        if addMenu:
            self.imageMenu.addAction(
                QAction("&" + name, self, triggered=lambda: self.displayImage(id))
            )
        if display:
    ...
    def displayImage(self, fileNameId):
        ...
        if image.isNull():
            QMessageBox.information(
                self, "Image Viewer", "Cannot load %s." % self.imgs[fileNameId]
            )
            return
    ...
    def addOption(self, n, id):
        ...
        """
        self.imageMenu.addAction(
            QAction("&" + n, self, triggered=lambda: self.displayImage(id))
        )
    ...
    def createActions(self):
        ...
        self.zoomInAct = QAction(
            "Zoom &In (25%)", self, shortcut="Up", enabled=False, triggered=self.zoomIn
        )
        ...
        self.displayPrevAct = QAction(
            "&Previous image",
            self,
            shortcut="Left",
            enabled=False,
            triggered=self.displayPrev,
        )
        self.displayNextAct = QAction(
            "&Next image",
            self,
            shortcut="Right",
            enabled=False,
        ...
        self.solveAct = QAction(
            ...
            shortcut="Ctrl+S",
            enabled=False,
            triggered=self.solve,
        )
        self.solveGreenAct = QAction(
            "&Solve puzzle (Green Background)",
            self,
            shortcut="Alt+S",
            enabled=False,
