...
def euclideanDistance(e1_lab_colors, e2_lab_colors):
    ...
    maximum = len1 if len1 < len2 else len2
    t1 = len1 / maximum
    t2 = len2 / maximum
...
def real_edge_compute(e1, e2):
    ...
    for col in e1.color:
        ...
        rgbs1.append(rgb)
        e1_lab_colors.append(
            color.rgb2lab([[[rgb[0] / 255.0, rgb[1] / 255.0, rgb[2] / 255.0]]])[0][0]
        )
        # Drop luminance
    ...
    return min(
        euclideanDistance(e1_lab_colors, e2_lab_colors),
        euclideanDistance(e1_lab_colors, e2_lab_colors[::-1]),
    )
